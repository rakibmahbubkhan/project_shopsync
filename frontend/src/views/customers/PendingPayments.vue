          ...
          <table class="w-full text-left">
            ...
            <tbody class="divide-y divide-slate-100">
              <tr v-for="customer in sortedAndFilteredCustomers" :key="customer.id" class="hover:bg-slate-50/50 transition-colors group">
                ...
                <td class="px-6 py-4 text-center">
                  <router-link 
                    :to="`/sales?customer_id=${customer.id}`" 
                    class="inline-flex items-center gap-1 px-3 py-1.5 bg-blue-50 text-blue-600 hover:bg-blue-100 rounded-lg text-sm font-medium transition-colors"
                  >
                    View Sales
                  ...
                  <button 
                    ...
                    class="inline-flex items-center gap-1 px-3 py-1.5 bg-green-50 text-green-600 hover:bg-green-100 rounded-lg text-sm font-medium transition-colors ml-2"
                  >
                    Collect Payment
                    <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <div v-if="showPaymentModal" class="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4" @click.self="showPaymentModal = false">
      <div class="bg-white rounded-2xl shadow-2xl w-full max-w-md animate-slide-up">
        <div class="p-5 border-b border-slate-100 flex justify-between items-center">
          ...
          <button @click="showPaymentModal = false" class="p-2 hover:bg-slate-100 rounded-lg transition-colors">
        ...
        <div class="p-5 space-y-4">
          ...
          <div class="pt-2">
            <button 
              ...
              :disabled="paymentAmount <= 0 || paymentAmount > selectedCustomerDue"
...
const processPayment = async () => {
  ...
  try {
    // This would call an API endpoint to record the payment
    // For now, we'll just show a success message
    alert(`Payment of ৳${formatNumber(paymentAmount.value)} collected from ${selectedCustomer.value.name}`);
