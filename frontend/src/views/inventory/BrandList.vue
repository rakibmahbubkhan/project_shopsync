  ...
  <div class="min-h-screen bg-gradient-to-br from-gray-50 to-indigo-50/30">
    <div class="container mx-auto px-3 sm:px-4 py-4 sm:py-6 md:py-8">
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-4 sm:p-6">
          <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            <div class="flex items-center gap-3">
              <div class="w-10 h-10 bg-gradient-to-br from-indigo-500 to-indigo-600 rounded-xl flex items-center justify-center shadow-lg shadow-indigo-200">
                <svg class="w-5 h-5 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-10L4 7m8 4v10M4 7v10l8 4" />
                </svg>
              </div>
              <div>
                <h1 class="text-xl sm:text-2xl font-bold text-gray-800">Brands</h1>
                <p class="text-xs sm:text-sm text-gray-500 mt-0.5">Manage your product brands and manufacturers</p>
              </div>
            </div>
            <button 
              @click="openModal()" 
              class="w-full sm:w-auto bg-gradient-to-r from-indigo-600 to-indigo-700 hover:from-indigo-700 hover:to-indigo-800 text-white px-4 sm:px-6 py-2.5 rounded-xl font-semibold transition-all duration-200 shadow-lg shadow-indigo-200 flex items-center justify-center gap-2 transform hover:scale-105 active:scale-95"
            >
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-4 hover:shadow-md transition-shadow">
          <div class="flex items-center justify-between">
            <div>
              <p class="text-xs text-gray-500 uppercase tracking-wider">Total Brands</p>
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-4 hover:shadow-md transition-shadow">
          <div class="flex items-center justify-between">
            <div>
              <p class="text-xs text-gray-500 uppercase tracking-wider">Active Brands</p>
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-4 hover:shadow-md transition-shadow">
          <div class="flex items-center justify-between">
            <div>
              ...
              <p class="text-2xl font-bold text-blue-600 mt-1">{{ brandsWithProducts }}</p>
            </div>
      ...
      <div v-else class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        >
          <div class="relative h-28 bg-gradient-to-r from-indigo-500 to-indigo-600">
            <div class="absolute inset-0 bg-black opacity-0 group-hover:opacity-10 transition-opacity duration-300"></div>
            <div class="absolute top-4 right-4 flex space-x-2">
              <button 
                @click="openModal(item)" 
                class="p-2 bg-white rounded-lg text-indigo-600 hover:bg-indigo-50 transition-colors duration-200 shadow-md"
                title="Edit"
              >
      ...
      <div v-if="showModal" class="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4" @click.self="showModal = false">
        <div class="bg-white rounded-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto shadow-2xl animate-slide-up">
          ...
          <div class="sticky top-0 bg-white border-b border-gray-100 px-6 py-4 flex justify-between items-center">
            <div>
              <h2 class="text-xl font-bold text-gray-800">{{ editMode ? 'Edit Brand' : 'Add New Brand' }}</h2>
              <p class="text-sm text-gray-500 mt-0.5">{{ editMode ? 'Update brand information' : 'Fill in the details to create a new brand' }}</p>
            </div>
          ...
          <form @submit.prevent="saveItem" class="p-6 space-y-5">
            <div>
              ...
              <input 
                v-model="form.name" 
                type="text" 
                class="w-full border-2 border-gray-200 p-3 rounded-xl focus:border-indigo-400 focus:ring-4 focus:ring-indigo-100 outline-none transition-all"
            ...
            <div>
              ...
              <textarea 
                ...
                rows="4"
                class="w-full border-2 border-gray-200 p-3 rounded-xl focus:border-indigo-400 focus:ring-4 focus:ring-indigo-100 outline-none transition-all resize-none"
            ...
            <div class="flex flex-col-reverse sm:flex-row justify-end gap-3 pt-4 border-t border-gray-100">
              ...
              <button 
                type="submit" 
                class="bg-gradient-to-r from-indigo-600 to-indigo-700 text-white px-6 py-2.5 rounded-xl font-semibold hover:from-indigo-700 hover:to-indigo-800 transition-all disabled:opacity-50 shadow-lg shadow-indigo-200"
                :disabled="saving"
              >
...
const brandsWithProducts = computed(() => {
  // This would need actual product count from API
  // For now, just return a random calculation or 0
  return items.value.filter(b => b.products_count > 0).length;
});
